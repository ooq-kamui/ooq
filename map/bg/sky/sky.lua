...
function Sky.plychara_pos(_s)
	...
	local t_pos = id.pos(plychara_id)
	return t_pos
end
...
function Sky.upd_tile(_s, gradation_idx)
	...
	local t_tile = _s:sky_tile_base() + gradation_idx - 1

	local max = Sky.sky_idx_max * Sky.gradation_idx_max

	if t_tile > max then t_tile = t_tile - max end
...
function Sky.dsp__(_s, val)
	...
	tilemap.set_constant("#sky", "tint", vmath.vector4(1, 1, 1, alph))
	_s._dsp = val
	-- log._("sky dsp__", val)
end
